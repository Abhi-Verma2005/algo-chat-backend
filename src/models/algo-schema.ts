...
// Enums
export const contestStatusEnum = pgEnum("ContestStatus", [
  "UPCOMING",
  "ACTIVE",
  "COMPLETED",
]);
...
export const questionPlatformEnum = pgEnum("QuestionPlatform", [
  "LEETCODE",
  "CODEFORCES",
  "CODECHEF",
]);
...
export const GroupPermission = pgTable(
  ...
  (table) => ({
    groupIdContestIdUnique: unique("GroupPermission_groupId_contestId_key").on(
      table.groupId,
      table.contestId
    ),
  })
...
export const Contest = pgTable(
  ...
  (table) => ({
    statusStartTimeIdx: index("Contest_status_startTime_idx").on(
      table.status,
      table.startTime
    ),
  })
...
export const GroupOnContest = pgTable(
  "GroupOnContest",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => ({
    groupIdContestIdUnique: unique("GroupOnContest_groupId_contestId_key").on(
      table.groupId,
      table.contestId
    ),
    contestIdScoreIdx: index("GroupOnContest_contestId_score_idx").on(
      table.contestId,
      table.score
    ),
  })
...
export const questions = pgTable(
  ...
  (table) => ({
    inArenaArenaAddedAtIdx: index("questions_inArena_arenaAddedAt_idx").on(
      table.inArena,
      table.arenaAddedAt
    ),
  })
...
export const Bookmark = pgTable(
  ...
  (table) => ({
    userIdQuestionIdUnique: unique("Bookmark_userId_questionId_key").on(
      table.userId,
      table.questionId
    ),
...
export const Hintnew = pgTable(
  ...
  (table) => ({
    tagHintIdSequenceUnique: unique("Hintnew_tagHintId_sequence_key").on(
      table.tagHintId,
      table.sequence
    ),
    tagHintIdSequenceIdx: index("Hintnew_tagHintId_sequence_idx").on(
      table.tagHintId,
      table.sequence
    ),
  })
...
export const QuestionOnContest = pgTable(
  "QuestionOnContest",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => ({
    contestIdQuestionIdUnique: unique(
      "QuestionOnContest_contestId_questionId_key"
    ).on(table.contestId, table.questionId),
  })
);

export const TagHint = pgTable(
  "TagHint",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => ({
    questionIdTagIdUnique: unique("TagHint_questionId_tagId_key").on(
      table.questionId,
      table.tagId
    ),
...
export const TagHintRating = pgTable(
  "TagHintRating",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => ({
    userIdTagHintIdUnique: unique("TagHintRating_userId_tagHintId_key").on(
      table.userId,
      table.tagHintId
    ),
    tagHintIdx: index("TagHintRating_tagHintId_idx").on(table.tagHintId),
    userIdIdx: index("TagHintRating_userId_idx").on(table.userId),
    tagHintIdIsHelpfulIdx: index("TagHintRating_tagHintId_isHelpful_idx").on(
      table.tagHintId,
      table.isHelpful
    ),
  })
...
export const Submission = pgTable(
  ...
  (table) => ({
    userIdContestIdIdx: index("Submission_userId_contestId_idx").on(
      table.userId,
      table.contestId
    ),
    contestIdScoreIdx: index("Submission_contestId_score_idx").on(
      table.contestId,
      table.score
    ),
  })
...
export const GroupApplication = pgTable(
  "GroupApplication",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => ({
    applicantIdGroupIdUnique: unique(
      "GroupApplication_applicantId_groupId_key"
    ).on(table.applicantId, table.groupId),
    groupIdStatusIdx: index("GroupApplication_groupId_status_idx").on(
      table.groupId,
      table.status
    ),
    applicantIdStatusIdx: index("GroupApplication_applicantId_status_idx").on(
      table.applicantId,
      table.status
    ),
  })
...
export const UserConfig = pgTable("UserConfig", {
  ...
  user_brief: text("user_brief").default(
    "Just started focusing on DSA to build problem-solving skills for placements. Motivated to improve, needs structured guidance and consistent practice to grow from basics to intermediate level."
  ),
  createdAt: timestamp("createdAt").defaultNow(),
  updatedAt: timestamp("updatedAt").defaultNow(),
});
...
export const _QuestionToQuestionTag = pgTable(
...
export const _QuestionToTemp = pgTable(
  "_QuestionToTemp",
  {
    A: varchar("A").notNull(), // questionId
...
export const _ContestPermissionToUser = pgTable(
  "_ContestPermissionToUser",
  {
    A: varchar("A").notNull(), // contestPermissionId
    B: varchar("B").notNull(), // userId
  },
  (table) => ({
    AB_unique: unique("_ContestPermissionToUser_AB_unique").on(
      table.A,
      table.B
    ),
...
export const tempContestQuestionRelations = relations(
  TempContestQuestion,
  ({ many }) => ({
...
export const tempContestTimeRelations = relations(
  TempContestTime,
  ({ one }) => ({
    user: one(User, {
...
export const questionOnContestRelations = relations(
  QuestionOnContest,
  ({ one }) => ({
    contest: one(Contest, {
...
export const contestPermissionRelations = relations(
  ContestPermission,
  ({ one, many }) => ({
    contest: one(Contest, {
...
export const groupApplicationRelations = relations(
  GroupApplication,
  ({ one }) => ({
    applicant: one(User, {
...
export const _QuestionToQuestionTagRelations = relations(
  _QuestionToQuestionTag,
  ({ one }) => ({
    question: one(questions, {
...
export const _QuestionToTempRelations = relations(
  _QuestionToTemp,
  ({ one }) => ({
    question: one(questions, {
...
export const questionToQuestionTag = pgTable(
  "_QuestionToQuestionTag",
  {
...
export const _ContestPermissionToUserRelations = relations(
  _ContestPermissionToUser,
  ({ one }) => ({
